...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, name, verifyUrl } = parsed.data;
    ...
    const response = await fetch("https://api.useplunk.com/v1/send", {
      ...
      body: JSON.stringify({
        to: email,
        subject: "Verifica este dispositivo en Orientia",
        body: `
          <!DOCTYPE html>
          <html>
            <head>
              <meta charset="utf-8">
              <title>Verifica dispositivo - Orientia</title>
            </head>
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="text-align: center; margin-bottom: 30px;">
                <h1 style="color: #059669; margin-bottom: 10px;">Orientia</h1>
                <p style="color: #666; font-size: 16px;">Verificacion de dispositivo</p>
              </div>

              <div style="background: #f8f9fa; padding: 30px; border-radius: 10px; margin-bottom: 30px;">
                <h2 style="color: #333; margin-bottom: 20px;">Hola ${name ?? "usuario"},</h2>
                <p style="margin-bottom: 20px;">
                  Hemos detectado un inicio de sesion desde un dispositivo nuevo. Para continuar, verifica este dispositivo:
                </p>
              ...
              <div style="text-align: center; color: #666; font-size: 12px;">
                ...
                <p>Si no has intentado iniciar sesion, ignora este email.</p>
              </div>
            </body>
          </html>
        `,
